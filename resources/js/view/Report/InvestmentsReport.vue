  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <v-col cols="12">
            <h1>{{ $vuetify.lang.t('$vuetify.page.InvestmentsReport.title') }}</h1>
          </v-col>
        </v-row>
        <report-viewer-component
            report-file="/reports/prt/FixedCapitalInvestment.mrt"
            :data="data"
            data-source="Invest"
        />
      </v-card-text>
  ...
  methods: {
    ...
    loadReportDataSet() {
      if (this.selectedCity && this.selectedDate) {
        this.$store.dispatch(reportActionType.fetchInvestmentsReportData, {
