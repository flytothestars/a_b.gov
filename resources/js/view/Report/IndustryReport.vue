  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <v-col cols="12">
            <h1>{{ $vuetify.lang.t('$vuetify.page.IndustryReport.title') }}</h1>
          </v-col>
        </v-row>
        <report-viewer-component
            report-file="/reports/prt/Promishlennost.mrt"
            :data="data"
            data-source="Promishlennost"
        />
      </v-card-text>
    </v-card>
  </v-container>
  ...
  methods: {
    ...
    loadReportDataSet() {
      if (this.selectedCity && this.selectedDate) {
        this.$store.dispatch(reportActionType.fetchIndustryReportData, {
